...
def load_partition_data_ImageNet(dataset, data_dir,
    ...
    train_data_num = len(train_dataset)
    test_data_num = int(len(test_dataset)*0.1)
    ...
    train_data_global, test_data_global = get_dataloader_ImageNet_truncated(train_dataset, test_dataset,
                                                                            train_bs=batch_size, test_bs=512,
    ...
    for client_idx in range(client_number):
        if client_number == 1000:
            ...
            data_local_num_dict[client_idx] = sum(class_num_dict[client_idx*10 + i] for i in range(10))
        elif client_number == 10:
            dataidxs = [client_idx * 100 + i for i in range(100)]
            data_local_num_dict[client_idx] = int(sum(class_num_dict[client_idx*100 + i] for i in range(100))*0.1)
        # elif client_number == 10:
        #      dataidxs = [client_idx * 100 + i for i in range(10)]
        #      data_local_num_dict[client_idx] = sum(class_num_dict[client_idx*100 + i] for i in range(10))
        else:
