...
def partition_data(dataset, datadir, partition, n_nets, alpha):
    ...
    elif partition == "hetero-fedcom":
        K = 10
        N = y_train.shape[0]
        logging.info("N = " + str(N))
        net_dataidx_map = {}
        class_idx = list(range(K))
        np.random.shuffle(class_idx)
        idx_batch = [[] for _ in range(20)]
        idx_batch_total = []
        for pos, k in enumerate(class_idx):
            idx_k = np.where(y_train == k)[0]
            np.random.shuffle(idx_k)
            if pos % 2 != 0:
                idx_batch= [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k, 20))]
            elif pos != 0:
                idx_batch_total+=idx_batch
                idx_batch = [[] for _ in range(20)]
        idx_batch_total+=idx_batch
        for j in range(n_nets):
            np.random.shuffle(idx_batch_total[j])
            net_dataidx_map[j] = idx_batch_total[j]
...
def load_partition_data_cifar10_fedcom(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
    ...
    train_data_global, test_data_global = get_dataloader(dataset, data_dir, batch_size, batch_size*16)
    logging.info("train_dl_global number = " + str(len(train_data_global)))
    logging.info("test_dl_global number = " + str(len(test_data_global)))
    ...
    for client_idx in range(client_number):
        ...
        # training batch size = 64; algorithms batch size = 32
        train_data_local, test_data_local = get_dataloader(dataset, data_dir, batch_size, batch_size*16,
                                                 dataidxs)
