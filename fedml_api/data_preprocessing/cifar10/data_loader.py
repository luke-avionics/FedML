...
def load_partition_data_cifar10(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size):
    ...
    train_data_global, test_data_global = get_dataloader(dataset, data_dir, batch_size, 256)
    logging.info("train_dl_global number = " + str(len(train_data_global)))
    logging.info("test_dl_global number = " + str(len(test_data_global)))
    ...
    for client_idx in range(client_number):
        ...
        # training batch size = 64; algorithms batch size = 32
        train_data_local, test_data_local = get_dataloader(dataset, data_dir, batch_size, 256,
                                                 dataidxs)
